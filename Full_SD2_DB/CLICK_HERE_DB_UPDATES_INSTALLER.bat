...
:install
set dbpath=sql_updates
set dbpath1=sql_updates
set mysql=.
...
for %%i in (%dbpath%\wb*mangos*sql) do if %%i neq %dbpath3%\wb*mangos*sql if %%i neq %dbpath1%\wb*mangos*sql if %%i neq %dbpath2%\wb*mangos*sql echo %%i & %mysql%\mysql -q -s -h %svr% --user=%user% --password=%pass% --port=%port% %wdb% < %%i
...
for %%i in (%dbpath1%\wb*scriptdev2*sql) do if %%i neq %dbpath%\wb*scriptdev2*sql if %%i neq %dbpath1%\wb*scriptdev2*sql if %%i neq %dbpath2%\wb*scriptdev2*sql echo %%i & %mysql%\mysql -q -s -h %svr% --user=%user% --password=%pass% --port=%port% %cdb% < %%i
